...
% Takes as input the vehicles and their optimal velocity
% As output we have the total fuel consumption

function [TFC, pos, yplot, Rhoplot, Rho, t] = Simulator_TFC(Rho,par,v,vel,pos)
...
% creates a vector of lanes and labels
for i=1:n
    lane(i)=v{i}.lane;
    labels(i)=v{i}.label;
end 
% deletes double values
l_tilde=unique(lane);

% creates a logic matrix of the lanes 
for i=1:length(l_tilde)
        s(i,:)=(lane==l_tilde(i)*ones(size(lane)));
end


    while T-par.hor_tot<=par.hor
        iter=iter+1;
        ...
        % this cicle verifies if there are two vehicle in the same cell and
        % then if they are in the same lane. If so, the upstream and faster vehicle will
        % merge with the downstream and slower one
        for i=1:length(m_tilde)
            m_logic=find(z(i,:)==1); % find the index of the vehicles in the same cell
            if length(m_logic) > 1   % if there are two vehicles in the same cell the lenght of m_logic will be > 1
                clear s_tilde
                for j=1:length(m_logic) 
                    s_tilde(:,j) = s(:,m_logic(j)); % creates a matrix logic matrix of same cell vehicles
                end
                [height,width] = size(s_tilde);
                for p=1:height 
                    s_logic=find(s_tilde(p,:)==1);
                    if length(s_logic)>1 % if there are two vehicles in the same lane the lenght of m_logic will be > 1
                        clear A
                        clear B
                        for j=1:length(s_logic)
                            A(j,1)=v{m_logic(s_logic(j))}.vel;    
                            B(j,1)=v{m_logic(s_logic(j))}.pos;                        
                        end
                        % Applies to all vehicle in the same cell and in
                        % the same lane, the higher position and the slower
                        % speed
                        for j=1:length(s_logic)
                    ...
                    end % VERIFICO SE HANNO LA STESSA VELOCITA E CAMBIARE ALPHA E FARE UPDATE
                end
            end
        end
        ...
        % RIMETTERE A TUTTI ALPHA ORIGINALE
