...
% prediction
% function returning the total fuel consumption to be optimized

function [TFC]=Optimizer_TFC_bayes_partiallycentralized(Rho,par,v,vel,pos,xx, v_neighbors,vel_neighbors,pos_neighbors)

v_group = [v v_neighbors];

v_group{1}.vel=xx{1,1};
v_group{1}.pos=pos(1);
update(v_group{1}, par);

for i = 2:length(v_neighbors)
    v_group{i}.vel=vel_neighbors(i);
    v_group{i}.pos=pos_neighbors(i);
    update(v_group{i}, par);
end
...
ATT=0;                                  %Average Travel Time
    ...
    for i=1:n
        lane(i)=v_group{i}.lane;
        labels(i) = v_group{i}.label;
    end 
    l_tilde=unique(lane);
    ...
    while T-par.hor_tot<=par.hor_opt 
                ...
                for p=1:height
                    ...
                    if length(s_logic)>1
                        ...
                        for j=1:length(s_logic)
                            A(j,1)=v_group{m_logic(s_logic(j))}.vel;    % crea una matrice con le velocità
                            B(j,1)=v_group{m_logic(s_logic(j))}.pos;                        
                        end
                        for j=1:length(s_logic)
                            v_group{m_logic(s_logic(j))}.vel=min(A(A>0));
                            v_group{m_logic(s_logic(j))}.pos=max(B(B>0));
                            update(v_group{m_logic(s_logic(j))}, par);
                        end
                    end
                end
            end
        ...
        for i=1:length(v_group)
            if v_group{i}.activity==1
                active1(i) = v_group{i}.label;
            elseif v_group{i}.activity==0
                notactive1(i) = v_group{i}.label;
            end
        end 
        ...
        for k = 1:length(active2)
            ...
            pos_act(k) = v_group{active3(k)}.pos;
        end
        ...
        for k = 1:length(notactive2)
            ...
            pos_notact(k) = v_group{notactive3(k)}.pos;
        end
        ...
        if (v_group{act(p)}.m<par.J)
           if (v_group{act(p)}.pos ~= x(1,v_group{act(p)}.m)) 
                if (flux(RS(Rho(1,v_group{act(p)}.m-1),Rho(1,v_group{act(p)}.m+1),par,v_group{act(p)}),par) > (v_group{act(p)}.F_alpha+v_group{act(p)}.vel*RS(Rho(1,v_group{act(p)}.m-1),Rho(1,v_group{act(p)}.m+1),par,v_group{act(p)}))) 
                    v_group{act(p)}.activity=1;
                    d = (v_group{act(p)}.Rho_check - Rho(1,v_group{act(p)}.m)) / (v_group{act(p)}.Rho_check - v_group{act(p)}.Rho_hat);
                    dtemp = par.dx_0*(1-d)/v_group{act(p)}.vel ; 
                    if d>=0 && d<=1
                        num_flux(v_group{act(p)}.m-1) = godunov(Rho(1,v_group{act(p)}.m-1),v_group{act(p)}.Rho_hat,par);
                        num_flux(v_group{act(p)}.m) = (min(dtemp,par.dt)*flux(v_group{act(p)}.Rho_check,par) + max(par.dt-dtemp,0)*flux(v_group{act(p)}.Rho_hat,par))/par.dt;
                    end
                    v_group{act(p)}.pos=v_group{act(p)}.pos+v_group{act(p)}.vel*par.dt;
                    v_group{act(p)}.m=find(v_group{act(p)}.pos>=x(1,:), 1, 'last' );
                elseif (flux(RS(Rho(1,v_group{act(p)}.m-1),Rho(1,v_group{act(p)}.m+1),par,v_group{act(p)}),par) <= (v_group{act(p)}.F_alpha+v_group{act(p)}.vel*RS(Rho(1,v_group{act(p)}.m-1),Rho(1,v_group{act(p)}.m+1),par,v_group{act(p)}))) && (Rho(1,v_group{act(p)}.m-1)<Rho(1,v_group{act(p)}.m+1))
                    v_group{act(p)}.activity=0;
                    k=v_group{act(p)}.m; % reconstruction of classical shock in cell m
                    l=(flux(Rho(1,k+1),par)-flux(Rho(1,k-1),par))/(Rho(1,k+1)-Rho(1,k-1));  %shock speed
                    dj=(Rho(1,k+1)-Rho(1,k))/(Rho(1,k+1)-Rho(1,k-1)); %reconstructed position in cell j
                    if l>0               
                        if dj>=0 && dj<=1
                    ...
                    v_group{act(p)}.pos=v_group{act(p)}.pos+Vbus(Rho(1,v_group{act(p)}.m),par,v_group{act(p)})*par.dt;
                    v_group{act(p)}.m=find(v_group{act(p)}.pos>=x(1,:), 1, 'last' );
            ...
            v_group{act(p)}.m=par.J;
            v_group{act(p)}.pos=min(par.L,v_group{act(p)}.pos+Vbus(Rho(1,v_group{act(p)}.m),par,v_group{act(p)})*par.dt) ;
        end

        yplot(act(p),iter)=v_group{act(p)}.pos;
        ...
        ATT=ATT+sum(tt(1,:)) * par.dx_0(1,1) * par.dt;
