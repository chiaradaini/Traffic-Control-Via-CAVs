...
v{1}=av(a*par.L,1,1,100,1); %black
updatealpha(v{1}, par);
update(v{1}, par);

v{2}=av(b*par.L,2,1,100,2); %blue
updatealpha(v{2}, par);
update(v{2}, par);

v{3}=av(c*par.L,3,1,60,3); %red
updatealpha(v{3}, par);
update(v{3}, par);

v{4}=av(d*par.L,3,1,120,4); %green
updatealpha(v{4}, par);
update(v{4}, par);
...
cfl = 0.9 / par.V_max;
par.dt = cfl * par.dx_0;
Enditer=round(par.Tf/par.dt);                       % Number of iterations
...
yplot=zeros(1,Enditer);
Rhoplot=zeros(Enditer,par.J);
num_flux=ones(1,par.J);                     % Inizialization of the numerical flux vector SIZE: J
tol = 1e-2;
iter=0;
T=0;
p=0;
...
for i=1:length(l_tilde)
    ...
    while T<=par.Tf 
        ...
        for i=1:length(m_tilde)
            m_logic=find(z(i,:)==1);
            if length(m_logic) > 1   % if there are n>1 % there are two vehicles or more in the same cell 
                clear s_tilde
                ...
                for p=1:height
                    ...
                    if length(s_logic)>1 % if n>1 the two vehicle are in the same lane and same cell
                        clear A
                        clear B
        ...
        Rho(1,:)=Rho(1,:)-lambda*(num_flux(1,:)-num_fluxm(1,:)); %density update
        
        % Total fuel consumption
        fc=fuel_consumption(Rho,par);
        TFCX = sum(fc ) * par.dx_0(1,1);
        TFC = TFC + sum(fc ) * par.dx_0(1,1) * par.dt;
        ...
        if (iter==1 || mod(iter,10)==0)
            ...
            figure(1);
            if n==1
                plot(x,Rho,'k',v{1}.pos,Rho(1,v{1}.m),'ko-');
            elseif n==2
                plot(x,Rho,'k',v{1}.pos,Rho(1,v{1}.m),'ko-',v{2}.pos,Rho(1,v{2}.m),'bo-');
            elseif n==3
                plot(x,Rho,'k',v{1}.pos,Rho(1,v{1}.m),'ko-',v{2}.pos,Rho(1,v{2}.m),'bo-',v{3}.pos,Rho(1,v{3}.m),'ro-');
            elseif n==4
                plot(x,Rho,'k',v{1}.pos,Rho(1,v{1}.m),'ko-',v{2}.pos,Rho(1,v{2}.m),'bo-',v{3}.pos,Rho(1,v{3}.m),'ro-',v{4}.pos,Rho(1,v{4}.m),'go-');
            end

            ylim([0,1.2*par.Rho_max]);
            title(['Time= ', num2str(t(iter,1))]);
            xlabel('Space');
            ylabel('Density');
            legend('Mean traffic density reconstruction', 'CAVs trajectory reconstruction');
...
% hold off
% 
% saveas(gca,'same_lanes_figure4_c.png');
